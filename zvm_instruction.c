...
uint8_t shift_amount = vm->cpu.R[right];
if(shift_amount >7){
        zvm_raise(vm, EXECUTE, INVALID_SHIFT_AMOUNT);
        return false;
}
   vm->cpu.R[output] = vm->cpu.R[left] << vm->cpu.R[right];
    printf("R=%u\n", vm->cpu.R[output]);
...
uint8_t target_address = vm->cpu.R[left];
if(target_address >= ZVM_PROGRAM_DEFAULT_CODE_SEGMENT_SIZE){
 zvm_raise(vm, EXECUTE, BAD_MEMORY_ADDRESS);
        return false;
}
    vm->cpu.IP = target_address;
printf("JUMPED TO IP=%u\n", vm->cpu.IP);
ZVM_INSTRUCTION_HANDLER_FUNCTION_END
